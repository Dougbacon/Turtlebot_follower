...
%              imshow(depthImage)
                ...
                if count >= 5 % If there are more than 5, the QR code detected is valid
                    ...
                    disp("QR detected")
                     ...
                     for i=1 : inlierDistorted.Count
                        ...
                        centroidPointU = mean(CorPoints(1,:)); % Get mean location of the points
                        centroidPointV = mean(CorPoints(2,:));
                        avgDepth = mean(CorPoints(3,:));       % Get mean depth of the matched points
            
                        centroidPoint = [centroidPointU centroidPointV];      % Mean Point
                        ...
                        turnAngle = (asind(-centroidPoint3D(1,1)/avgDepth));  % Calculate Angle needed to face QR
                        ...
                        angledif = follTheta + turnAngle; % Angle facing QR
            
                        Xval = avgDepth*cos(deg2rad(angledif));
                        Yval = avgDepth*sin(deg2rad(angledif));
            
                        % Calculate the location of the QR
                        
                        leadPose = rosmessage("geometry_msgs/Pose","DataFormat","struct");
                        angledifquat = rosmessage("geometry_msgs/Quaternion","DataFormat","struct");
                        angledifEul = eul2quat([deg2rad(angledif) 0 0]);
                        angledifquat.W = angledifEul(1);
                        angledifquat.X = angledifEul(2);
                        angledifquat.Y = angledifEul(3);
                        angledifquat.Z = angledifEul(4);
        ...
        function InterPose = generateIntersectPoint(self,LeadPose)
            ...
            LeadQuat = LeadPose.Orientation;
            LeadAngle = quat2eul([LeadQuat.W LeadQuat.X LeadQuat.Y LeadQuat.Z]);
            LeadTheta = rad2deg(LeadAngle(1));

            dist = sqrt((LeadPose.Pose.Pose.Position.X - follPose.X)^2 + (LeadPose.Pose.Pose.Position.Y - follPose.Y)^2); % Get distance between the the leader and the follower
       
            distError = dist - 0.5; % Get new certain distance
            ...
            InterPose.Orientation = rosmessage("geometry_msgs/Quaternion","DataFormat","struct");
            InterPose.Pose.Pose.Position.X = follPose.X + InterX;
            InterPose.Pose.Pose.Position.Y = follPose.Y + InterY;
            InterPose.Orientation = LeadPose.Orientation;
...
%         
        ...
        function atPose = checkPosition(self,InterPose)
            ...
            % Check Distance bwtween intersection point and the follower 
        ...
        function [angleDiff, direction]= checkPositionToQR(self,LeadPose)
           ...
           direction = 0;
            angleDiff= LeadTheta-theta1; % Get the difference between each robot
            if angleDiff < 0
        ...
        function rotateToQR(self,LeadPose)

           [angle,direction]=self.checkPositionToQR(LeadPose); %Get the angle between robots and the rotating direction
            if abs(angle) >= 0.5 && direction ==1 %Check if angle greater than 0.5 turn anti-clockwise
                ...
                [angle,direction]=self.checkPositionToQR(LeadPose);
